...
module.exports.buildFuncs = function buildFuncs(code) {
    ...
    function _buildGenerateCreate(generatorPath, docText) {

        var match = regexFindInitInDoc.exec(docText);
        if (!match)
            return null;

        var initStatement = '\n(function ' + generatorPath.replace(/\./g, '_') + '_init() {\n';
        ...
        do {
            ...
            var initParams = _quoteSensitiveStringSplit(initCode).map(_trim).filter(_filterEmpty);
            // Build out test
            initStatement += '\tDTestGenerateCreate(\'' + generatorPath + '\', [' + initParams.map(JSON.stringify).join(', ') + ']);\n';
            // Keep track
            initCalls++;
            // Update docText window
            docText = docText.substr(match.index + match[0].length);
    ...
    function processGeneratorInitCallExpression(expression, generatorPath, leadingComments) {

        console.log('processGeneratorInitCallExpression', expression, generatorPath, leadingComments)

        // Check for generator.js syntax.
        var chain = [],
            callee = expression.callee,
            member = callee,
            property, init,
            docText = leadingComments || expression.leadingComments;
        ...
        if (property === 'generate') {
            // generator init
            if (init = _buildGenerateCreate(generatorPath, docText)) {
                inits.push(init);
                return true;
            }
        }
    ...
    function processGeneratorDefineCallExpression(expression) {
        // Check for generator.js syntax.
        var chain = [],
            callee = expression.callee,
        ...
        if (property === 'definePrototype') {
            recursivelyTraverse((args[1] || args[0]).properties, chain.join('.') + '.proto.');
            return true;
        }
        ...
        for (; i < len; i++) {
            ...
            switch (node.type) {
                ...
                case 'VariableDeclarator':
                    ...
                    switch (node.init.type) {
                        ...
                        case 'FunctionExpression':
                            ...
                            break;
                        case 'CallExpression':
                            processGeneratorInitCallExpression(
                                node.init,
                                node.id.name,
                                node.leadingComments || node.id.leadingComments || leadingComments);
                            break;
                ...
                case 'ExpressionStatement':
                    ...
                    if (node.expression.type === 'AssignmentExpression' && node.expression.right.type === 'FunctionExpression') {
                    ...
                    } else if (node.expression.type === 'AssignmentExpression' && node.expression.right.type === 'CallExpression') {
                        // Traverse left member loop to resolve object heirarchy
                        var chain = [],
                            expression = node.expression,
                            left = expression.left,
                        ...
                        processGeneratorInitCallExpression(expression.right, execPath + '.' + property, node.leadingComments || expression.leadingComments || expression.left.leadingComments);
                    } else if (node.expression.type === 'CallExpression') {
                        processGeneratorDefineCallExpression(node.expression);
                    }
