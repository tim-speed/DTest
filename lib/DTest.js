...
module.exports.buildFuncs = function buildFuncs(code) {
    ...
    while (match = regexFindTest.exec(codeWindow)) {
        var fullText = match[0],
            ...
            funcName = match[2],
            funcParams = match[3].split(',').map(_trim).filter(_filterEmpty);

        //console.log('found match', match);
    
        // Append everything before the function and update the window
        codeOut += codeWindow.substr(0, match.index);
        codeWindow = codeWindow.substr(match.index);
        ...
        codeOut += codeWindow.substr(0, fullText.length);
        codeWindow = codeWindow.substr(fullText.length);
    }
