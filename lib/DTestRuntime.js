...
(function() {
    ...
    glob.DTestAssert = function DTestAssert(funcPath, funcArgs, testType, comparison, me, initPath, remainingPath) {
        ...
        for (; i < endIndex; i++) {
            ...
            if ((funcName === 'prototype' || funcName === 'proto') && i > 0) {
                ...
                for (ppI = 0; ppI < i; ppI++) {
                    ...
                    if (!ppItem) {
                        return fail('No @init directives defined for ' + pathParts[i - 1] +
                ...
                for (; i < len; i++) {
                    ...
                    if (arg[0] === '(') {
                        ...
                        tests = tests.map(function asyncTestArgMap(val) {
                            return _quoteSensitiveStringSplit(val).map(_trim).filter(_filterEmpty);
                        });
