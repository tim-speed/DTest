...
(function() {
    ...
    glob.DTestAssert = function DTestAssert(thisParam, funcName, funcArgs, testType, comparison) {
        var me = (thisParam || glob),
            testString = funcArgs.join(' ') + ' ' + testType + (comparison ? (' ' + comparison) : ''),
            func = me[funcName];

        if (!func) {
            return fail(funcName + ' is not defined on type ' + me.constructor.name + ' - for test: ' + testString);
        }
        ...
        var funcPath = me.constructor.name + '.' + funcName,
            stdMessage = funcPath + ' - test: ' + testString,
